...
def test_seed_dev_data_imports_reports_and_is_idempotent(
    ...
    assert first_result.imported == 7
    assert first_result.duplicates == 0
    assert second_result.imported == 0
    assert second_result.duplicates == 7
    assert _count(session, Runner) == 4
    assert _count(session, CaseReportModel) == 7
    ...
    blocked_report = session.scalar(
        select(CaseReportModel).where(CaseReportModel.idempotency_key == "dev-seed-payment-blocked")
    )
